...
//! reviewer looks at the real layout, not at a drawing of it. The scene catalog and the
//! rasteriser are shared with the `goldens` test (see `examples/common/scenes.rs`), so what
//! you eyeball here is exactly what the goldens lock in.
//!
...
use pomodoro_display::SCREEN_SIZE;
