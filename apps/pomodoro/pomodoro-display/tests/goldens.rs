//! Golden screens — the committed picture of every pomodoro state, guarded against drift.
//!
//! Every screen in the shared catalog (`examples/common/scenes.rs`, the same one the
//! `pomodoro-screenshots` example renders) is rasterised through `pomodoro_display::render` and
//! compared byte-for-byte against a committed reference PNG under `pomodoro-display/goldens/`.
