//! Golden screens — the committed picture of every orientation pose, guarded against drift.
//!
//! Every screen in the shared catalog (`examples/common/scenes.rs`, the same one the
//! `orientation-screenshots` example renders) is rasterised through `orientation_display::render` and
//! compared byte-for-byte against a committed reference PNG under `orientation-display/goldens/`.
//! A change to the layout, the wording, the colours, the bars or the named face flips the
