...
//!
//! The grid ([`crate::grid`]) is a pure function of a phase `φ`; this is where `φ` comes from.
//! The original advances its breath by a small step every `draw()` call; here the breath is a
//! **continuous** function of time instead, so the render loop may repaint as fast as the
//! hardware allows and the grid neither speeds up nor slows down — a faster repaint only samples
//! the same breath more finely, which is the whole of what "smoother" means.
//!
//! ## Why it wraps to one turn
//!
//! Every use of `φ` in the grid is inside a sine, so `φ` and `φ + 2π` are the same picture — the
//! whole grid returns to its start after one turn. Left to grow, `φ` would reach thousands of
...
//! handed to the grid is always in `[0, 2π)`, exact, however long the animation has been running.

use core::f32::consts::TAU;

/// One full breath of the grid, in milliseconds: the time for the phase to sweep one turn and the
/// whole pattern to return to where it began.
///
/// Chosen for a calm breath — a little over three seconds a cycle, close to the source's own
/// speed. The clock is reduced modulo this before it becomes a float, so the phase never leaves
/// `[0, 2π)` and an `f32` never loses the low bits of a long-running phase.
pub const PERIOD_MS: u64 = 3_200;
...
/// can compare two moments for equality, and a full period later reads exactly the same. The
/// clock is reduced modulo [`PERIOD_MS`] as an integer *before* the multiply, so the phase is on
/// the ring however long the animation has run.
...
mod tests {
    ...
    fn the_phase_stays_on_the_ring() {
        // A day of milliseconds, sampled on a prime stride so the samples do not all land on a
        // period boundary.
