...
//! ```
//!
//! The pixels come from `plant_display::render` — the same function the ST7789 adapter calls on
...
//! the `goldens` test (see `examples/common/scenes.rs`), so what you eyeball here is exactly what
//! the goldens lock in.
//!
...
//! however the glass is wired. To answer *that* question, flash `just run-bin
//! display-colour-check` and look at the board. See the crate docs.
