//! `wifi` — 2.4 GHz station bring-up and keep-alive.
//!
...
//! inward domain port for "WiFi" — an app's TCP consumers (the native-API server,
//! the HTTP metrics scraper) sit on top of the working stack this brings up, so it
//! lives here in the shared `net` crate, not among any app's driven `adapters`.
//!
...
//! `WIFI_PASSWORD` env read below. A future Noise PSK follows the same path.
//!
...
/// breaks).
const SSID: &str = env!("WIFI_SSID", "WIFI_SSID not set — net/build.rs must emit it");
const PASSWORD: &str = env!(
    "WIFI_PASSWORD",
    "WIFI_PASSWORD not set — net/build.rs must emit it"
);
...
/// (the native-API server, the metrics scraper); the composition root holds it
/// accordingly.
...
impl<'d> WifiStation<'d> {
    ...
    ///
    /// A composition root advertises this as the ESPHome `mac` TXT record — the
    /// identifier Home Assistant keys the device on. Read from the driver, so it
    /// reflects the actual radio, not a guess.
    ...
    ///
    /// A networked monitor must survive a router reboot without a power cycle.
    /// ESP-IDF does not reconnect on its own, so the caller ticks this from its
    /// loop: while associated and holding an IP it returns immediately; once the
    /// AP comes back it re-associates and waits for a fresh lease.
...
/// the resolver path — the DNS servers the lease handed us actually answer. Kept
/// infrastructure-side because it exercises the ESP netif's socket stack, the same
/// stack an app's TCP client binds.
