...
//! *electrical* concern the driven adapter owns, distinct from the domain's
//! temporal sampling policy ([`plant_core::sampler::step`], which folds readings
//! taken seconds apart). Kept pure and hardware-free so the reduction is
//! unit-tested on the host, away from the on-target HAL.
...
/// `u32`.
///
// Off the ESP-IDF target the only non-test caller ([`crate::adc`]) is gated out,
// so the host test build sees this as dead outside `#[cfg(test)]`. It is not —
// it is the on-target adapter's inner loop.
#[cfg_attr(not(target_os = "espidf"), allow(dead_code))]
pub(crate) fn oversampled_mean<E>(
    samples: NonZeroU16,
...
mod tests {
    ...
    /// Sample counts in these tests are always literal and nonzero; the port's
    /// "empty average" case is the type itself — [`NonZeroU16`] makes zero
