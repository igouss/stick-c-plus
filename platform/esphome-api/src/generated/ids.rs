// @generated from proto/api.proto (aioesphomeapi @ 1e16d71). The message
// type id is the `option (id) = N` annotation on each API message. Do not
// hand-edit; regenerate per PROVENANCE.md. Sorted by id, so a binary search
// over the ids is valid.

/// Every ESPHome native-API message paired with its wire type id, sorted by
/// id. The single source of truth the [`message_name`](crate::message_name)
/// and [`message_id`](crate::message_id) lookups read from.
pub const MESSAGE_IDS: &[(u32, &str)] = &[
    (1, "HelloRequest"),
    (2, "HelloResponse"),
